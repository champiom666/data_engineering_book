...
本章合并多图表信息图推理与医学图像工具调用两个专项案例，讨论视觉数据如何从对象识别走向证据组织和可执行推理。多图表信息图强调跨子图表证据聚合、数值关系和多步推理链路，MedImage-ToolVQA 强调 ROI、mask、bbox 与工具调用轨迹。两类任务都要求数据集同时记录视觉证据、问题结构、推理过程和人工复核边界。
